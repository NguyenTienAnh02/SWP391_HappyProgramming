...
import { MENTOR_ROLE, ADMIN_ROLE, MENTEE_ROLE } from "../constants/constant";
...
function Navbar() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        setUserId(sessionStorage.getItem("idne"));
  ...
  return (
    <>
      <nav
        className="navbar navbar-expand-sm navbar-light bg-light fixed-top nsvj nav-header"
        style={{ zIndex: "200" }}
      >
                  ...
                  {token ? (
                    <>
                      ...
                      {role === "USER_MENTOR" ? (
                        <Link
                          className="dropdown-item"
                          to={`/requestmentor/` + mentor.mentorID}
                          style={{ textDecoration: "none", fontSize: "18px" }}
                        >
                          <DashboardIcon />
                          Mentor's Manager
